...
- 2026-08-20 — Pin the standalone repository to Node 24 and upgrade `better-sqlite3` from 9.x to compatible 12.x — considered retaining the old native dependency or pinning an end-of-life Node runtime — because a clean install failed under Node 24 and Render now defaults new services to Node 24; the current 12.x release installs and opens SQLite successfully without application API changes — logged by @lead/vector [codex].
