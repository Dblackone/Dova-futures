...
def _configure_console_encoding() -> None:
    """Keep status logging from crashing the worker on non-UTF-8 consoles."""
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        self.core_gateway   = create_local_gateway(BASE_DIR)
        self.ui.on_text_command = self._on_text_command
        self.ui.on_core_approval = self._approve_core_action

    def _on_text_command(self, text: str):
        handled, result = self.core_gateway.submit_local_command(text)
        if handled:
            if result and result.success:
                self.ui.write_log(f"CORE: {result.output}")
            elif result and result.requires_approval:
                self.ui.write_log("CORE: Approval required before this action can run.")
                self.ui.set_core_approval_available(True)
            elif result:
    ...
    def _approve_core_action(self):
        result = self.core_gateway.approve_pending()
        self.ui.set_core_approval_available(False)
        if result.success:
            self.ui.write_log(f"CORE: {result.output}")
        else:
            self.ui.write_log(f"CORE ERR: {result.error}")

    def _report_send_error(self, future) -> None:
        try:
            future.result()
        except Exception as exc:
            self.ui.write_log(f"ERR: Could not send command — {str(exc)[:160]}")
