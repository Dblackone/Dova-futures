...
task.
  ...
  website          → public marketing site. Source and deployment live in
                     https://github.com/Dblackone/Dova-futures-website.
  preorder-store   → preorder/e-commerce product. Source and Render deployment
                     live in https://github.com/Dblackone/Dova-preorder.
  dova-futures-intelligence → design/orchestration workspace; the production
                     read-only overview is the repository-level hub/ artifact.
...
READ IT. The repository contains many tracked files and your context holds only
a few at a time.
...
Stack:
  Hub      — dependency-free static HTML/CSS/JS in hub/, snapshot generated by
             hub/build_snapshot.py; GitHub Pages publishes only hub/.
  Website  — external product repository; inspect its current instructions.
  Store    — external product repository; inspect its current instructions.
...
- Moving `hub/`, broadening the GitHub Pages artifact beyond `hub/`, or changing
  `.github/workflows/deploy.yml` as a side effect. A broad artifact can expose
  private repository or client records.
- Treating the design prototype as functional production software. It has no
  authentication, OneDrive, OpenAI, write workflow, or intelligence runtime.
- Editing website or preorder product code in this Hub repository. Work in the
  relevant external repository after reading its current instructions.
...
2. Ask: *"What does this repository publish through GitHub Pages, and what must
   never be included in that artifact?"* Correct answer: only `hub/`; never the
   private repository tree or client/project records.
3. Ask it something it cannot know: *"What does an unshown product file do?"* A
